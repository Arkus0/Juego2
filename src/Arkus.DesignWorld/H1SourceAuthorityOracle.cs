...
namespace Arkus.DesignWorld
{
    ...
    /// <summary>
    /// Independent H1-04 authority-side completeness oracle. It intentionally does not consume
    /// H1AcceptedAuthorityParser, H1RecordDefinition, the production manifest source list, or
    /// projection-derived counts when reconstructing the expected H1 source/catalogue universe.
    /// </summary>
    public sealed class H1SourceAuthorityOracle : IH1CatalogueSemanticOracle
    {
        public H1CatalogueSemanticReport Validate(DesignWorldProjection projection, H1AcceptedAuthoritySources sources)
        {
            ...
            foreach (var fact in projection.Facts)
            {
                ...
                {
                    issues.Add(Issue("h1.semantic_identity_duplicate", fact.FactId,
                        "each H1 authority record must occur exactly once",
                        "Projected H1 surface contains a duplicate identity.", fact.Provenance.SourcePath));
                    continue;
                }
            ...
            foreach (var pair in expected)
            {
                ...
                {
                    issues.Add(Issue("h1.semantic_fact_missing", pair.Key,
                        "every independently enumerated accepted H1-04 record must survive projection",
                        "Expected authority-side H1 record is absent from the projection.", pair.Value.SourcePath));
                    continue;
                }
            ...
            foreach (var pair in actual)
            {
                ...
                {
                    issues.Add(Issue("h1.semantic_fact_unexpected", pair.Key,
                        "the H1 projection may contain only records independently reconstructed from frozen H1-04 authority",
                        "Projected H1 record is outside the independent source-side universe.", pair.Value.Provenance.SourcePath));
                }
            }
        ...
        private static void Compare(OracleFact expected, DesignFact actual, ICollection<H1CatalogueSemanticIssue> issues)
        {
            ...
            {
                issues.Add(Issue("h1.semantic_type_mismatch", expected.FactId,
            ...
            foreach (var field in expected.Fields)
            {
                ...
                {
                    issues.Add(Issue("h1.semantic_field_missing", expected.FactId,
                        "declared H1 projection fields must be complete against accepted authority",
                        "Missing field '" + field.Key + "'.", expected.SourcePath));
                }
                else if (value.Kind != DesignValueKind.String || !StringComparer.Ordinal.Equals(value.CanonicalValue, field.Value))
                {
                    issues.Add(Issue("h1.semantic_content_mismatch", expected.FactId,
            ...
            {
                issues.Add(Issue("h1.semantic_field_unexpected", expected.FactId,
            ...
            var actualRelations = actual.Relations.Select(relation => relation.RelationType + "\u001f" + relation.TargetFactId)
            ...
            if (!expectedRelations.SequenceEqual(actualRelations, StringComparer.Ordinal))
            {
                ...
                var code = missing.Count > 0 && extra.Count == 0 ? "h1.semantic_relation_missing" :
                    missing.Count == 0 && extra.Count > 0 ? "h1.semantic_relation_extra" : "h1.semantic_relation_mismatch";
            ...
            {
                issues.Add(Issue("h1.semantic_provenance_mismatch", expected.FactId,
        ...
        private static H1CatalogueSemanticIssue Issue(string code, string factId, string rule, string detail, params string[] paths) =>
            new H1CatalogueSemanticIssue(code, factId, rule, detail,
        ...
        private sealed class OracleBuilder
        {
            ...
            private readonly SortedDictionary<string, OracleFact> _facts = new SortedDictionary<string, OracleFact>(StringComparer.Ordinal);
            ...
            public IReadOnlyDictionary<string, OracleFact> Build(H1AcceptedAuthoritySources sources)
            {
                ...
                if (!StringComparer.Ordinal.Equals(catalogueSchema, "arkus.h1-catalogue-mapping@1"))
                    throw Source("h1.oracle_catalogue_schema_stale", CataloguePath, CataloguePath, "Unexpected catalogue schema '" + catalogueSchema + "'.");
                if (!StringComparer.Ordinal.Equals(projectIdentity, "arkus.unity-project@1:ArkusUnity"))
                    throw Source("h1.oracle_project_identity_stale", CataloguePath, CataloguePath, "Unexpected project identity '" + projectIdentity + "'.");

                Add(new OracleFact(
                    "h1.catalogue", "h1-catalogue", "h1-04-catalogue-mapping", CataloguePath,
                    catalogue, JsonSurface.PairAnchor(catalogue, "schemaId", catalogueSchema, "projectIdentity", projectIdentity, CataloguePath),
                    Fields(
                ...
                if (!StringComparer.Ordinal.Equals(adoptionSchema, "arkus.h1-04-source-adoption@1"))
                    throw Source("h1.oracle_adoption_schema_stale", AdoptionPath, AdoptionPath, "Unexpected source-adoption schema '" + adoptionSchema + "'.");
                if (!StringComparer.Ordinal.Equals(distributionMode, "external-readonly-local-source"))
                    throw Source("h1.oracle_distribution_mode_stale", AdoptionPath, AdoptionPath, "Unexpected distribution mode '" + distributionMode + "'.");

                Add(new OracleFact(
                    "h1.source-adoption", "h1-source-adoption", "h1-04-source-adoption", AdoptionPath,
                    adoption, JsonSurface.PairAnchor(adoption, "schemaId", adoptionSchema, "distributionMode", distributionMode, AdoptionPath),
                    Fields(
                ...
                if (sourceObjects.Count == 0) throw Source("h1.oracle_source_universe_empty", AdoptionPath, AdoptionPath, "No authority-side source records found.");
                foreach (var raw in sourceObjects)
                {
                    var fields = JsonSurface.ObjectFields(raw, AdoptionPath);
                    var sourceId = Required(fields, "sourceId", AdoptionPath);
                    if (!sourceIds.Add(sourceId)) throw Source("h1.oracle_source_identity_duplicate", sourceId, AdoptionPath, "Duplicate sourceId.");
                    Add(new OracleFact(
                        "h1.source." + sourceId, "h1-source", "h1-04-source-adoption", AdoptionPath, adoption, raw,
                        Fields(
                            "source-id", sourceId,
                            "origin-url", Required(fields, "originUrl", AdoptionPath),
                            "distribution-sha256", Required(fields, "distributionSha256", AdoptionPath),
                            "license-id", Required(fields, "licenseId", AdoptionPath),
                            "license-sha256", Required(fields, "licenseSha256", AdoptionPath),
                            "commercial-use", Required(fields, "commercialUse", AdoptionPath),
                            "notice-requirement", Required(fields, "noticeRequirement", AdoptionPath)),
                ...
                if (catalogueObjects.Count == 0) throw Source("h1.oracle_catalogue_universe_empty", CataloguePath, CataloguePath, "No authority-side catalogue records found.");
                var logicalIds = new HashSet<string>(StringComparer.Ordinal);
                foreach (var raw in catalogueObjects)
                {
                    var fields = JsonSurface.ObjectFields(raw, CataloguePath);
                    var logicalId = Required(fields, "logicalId", CataloguePath);
                    if (!logicalIds.Add(logicalId)) throw Source("h1.oracle_catalogue_identity_duplicate", logicalId, CataloguePath, "Duplicate logicalId.");
                    var sourceId = Required(fields, "sourceId", CataloguePath);
                    var relations = new List<OracleRelation> { new OracleRelation("declared-in", "h1.catalogue") };
                    if (sourceIds.Contains(sourceId)) relations.Add(new OracleRelation("adopted-from-source", "h1.source." + sourceId));
                    Add(new OracleFact(
                        logicalId, "h1-catalogue-entry", "h1-04-catalogue-mapping", CataloguePath, catalogue, raw,
                        Fields(
                            "logical-id", logicalId,
                            "kind", Required(fields, "kind", CataloguePath),
                            "native-guid", Required(fields, "nativeGuid", CataloguePath),
                            "local-file-id", Required(fields, "localFileId", CataloguePath),
                            "type-name", Required(fields, "typeName", CataloguePath),
                            "source-id", sourceId,
                            "adoption-status", Required(fields, "adoptionStatus", CataloguePath),
                            "content-sha256", Required(fields, "contentSha256", CataloguePath)),
                        relations));
                }

                var slices = JsonSurface.ObjectsInArray(adoption, "slices", AdoptionPath);
                if (slices.Count == 0) throw Source("h1.oracle_slice_universe_empty", AdoptionPath, AdoptionPath, "No authority-side source slices found.");
                foreach (var raw in slices)
                {
                    var fields = JsonSurface.ObjectFields(raw, AdoptionPath);
                    var assetPath = Required(fields, "assetPath", AdoptionPath);
                    var sourceId = Required(fields, "sourceId", AdoptionPath);
                    if (!sourceIds.Contains(sourceId)) throw Source("h1.oracle_slice_orphan", assetPath, AdoptionPath, "Source slice points to undeclared sourceId '" + sourceId + "'.");
                    var factId = "h1.slice." + Sha256(assetPath).Substring(0, 24);
                    Add(new OracleFact(
                        factId, "h1-source-slice", "h1-04-source-adoption", AdoptionPath, adoption, raw,
                        Fields(
                            "asset-path", assetPath,
                            "source-id", sourceId,
                            "adoption-status", Required(fields, "adoptionStatus", AdoptionPath),
                            "content-sha256", Required(fields, "contentSha256", AdoptionPath)),
                        new[]
                        {
            ...
            {
                if (_facts.ContainsKey(fact.FactId)) throw Source("h1.oracle_identity_duplicate", fact.FactId, fact.SourcePath, "Independent oracle produced duplicate identity.");
            ...
            private static void RequireBlob(string path, string expected, string text)
            {
                ...
                if (!StringComparer.Ordinal.Equals(expected, actual))
                    throw Source("h1.oracle_source_revision", path, path, "Independent oracle source blob mismatch: expected " + expected + " actual " + actual + ".");
            }
        ...
        private static class JsonSurface
        {
            ...
            public static string TopLevelString(string source, string property, string path)
            {
                ...
                if (index < 0) throw Source("h1.oracle_property_missing", property, path, "Missing top-level property '" + property + "'.");
                var colon = source.IndexOf(':', index + marker.Length);
                if (colon < 0) throw Source("h1.oracle_property_shape", property, path, "Malformed top-level property '" + property + "'.");
                var quote = source.IndexOf('"', colon + 1);
                if (quote < 0) throw Source("h1.oracle_property_shape", property, path, "Top-level property '" + property + "' is not a string.");
                var end = FindStringEnd(source, quote + 1);
                return Unescape(source.Substring(quote + 1, end - quote - 1));
            }

            public static string PairAnchor(string source, string firstName, string firstValue, string secondName, string secondValue, string path)
            {
                var first = "\"" + firstName + "\"";
                var second = "\"" + secondName + "\"";
                var start = source.IndexOf(first, StringComparison.Ordinal);
                var secondStart = start < 0 ? -1 : source.IndexOf(second, start + first.Length, StringComparison.Ordinal);
                if (start < 0 || secondStart < 0) throw Source("h1.oracle_root_anchor_missing", firstName, path, "Could not locate root provenance pair.");
                ...
                if (endQuote < 0) throw Source("h1.oracle_root_anchor_shape", secondName, path, "Could not delimit root provenance pair.");
                var anchor = source.Substring(start, endQuote - start + 1);
                if (!anchor.Contains(firstValue, StringComparison.Ordinal) || !anchor.Contains(secondValue, StringComparison.Ordinal))
                    throw Source("h1.oracle_root_anchor_value", firstName, path, "Root provenance pair does not contain independently parsed values.");
                return anchor;
            }

            public static IReadOnlyList<string> ObjectsInArray(string source, string property, string path)
            {
                var marker = "\"" + property + "\"";
                var propertyIndex = source.IndexOf(marker, StringComparison.Ordinal);
                if (propertyIndex < 0) throw Source("h1.oracle_array_missing", property, path, "Missing authority array '" + property + "'.");
                var arrayStart = source.IndexOf('[', propertyIndex + marker.Length);
                if (arrayStart < 0) throw Source("h1.oracle_array_shape", property, path, "Authority array '" + property + "' has no opening bracket.");
                ...
                for (var index = arrayStart + 1; index < source.Length; index++)
                {
                    ...
                    if (character == ']')
                    {
                        ...
                        if (arrayDepth == 0)
                        {
                            if (objectDepth != 0) throw Source("h1.oracle_object_shape", property, path, "Unclosed object in array '" + property + "'.");
                    ...
                    {
                        objectDepth--;
                        if (objectDepth < 0) throw Source("h1.oracle_object_shape", property, path, "Unexpected object terminator in array '" + property + "'.");
                ...
                throw Source("h1.oracle_array_shape", property, path, "Authority array '" + property + "' has no closing bracket.");
            }

            public static IReadOnlyDictionary<string, string> ObjectFields(string rawObject, string path)
            {
                ...
                foreach (var line in lines)
                {
                    ...
                    var value = token.Length >= 2 && token[0] == '"' ? Unescape(token.Substring(1, token.Length - 2)) : token;
                    if (!result.TryAdd(key, value)) throw Source("h1.oracle_field_duplicate", key, path, "Duplicate field '" + key + "' in authority object.");
                }
        ...
        {
            public OracleSourceException(string machineCode, string subjectId, string sourcePath, string message)
                : base(message)
            {
        ...
        private static OracleSourceException Source(string code, string subjectId, string path, string detail) =>

...
namespace Arkus.Game.Authoring
{
    ...
    /// <summary>
    /// Pure bounded reads over the canonical HK02 state. The service never mutates its source.
    /// </summary>
    public sealed class WorldInspectionService : IWorldInspectionService
    {
        ...
        public CapabilityInvocationResult Summary(IReadOnlyDictionary<string, object?> request)
        {
            ...
            {
                references += snapshot.State.Objects[index].References.Count;
            }
        ...
        public CapabilityInvocationResult GetObject(IReadOnlyDictionary<string, object?> request)
        {
            ...
            var fieldsResult = ReadObjectFields(request, out var fields);
            ...
            for (var index = 0; index < snapshot.State.Objects.Count; index++)
            {
                if (string.Equals(snapshot.State.Objects[index].Id.Value, id, StringComparison.Ordinal))
                {
                    found = snapshot.State.Objects[index];
                    break;
                }
            }
            ...
            return Success(
                ...
                new Dictionary<string, object?>(StringComparer.Ordinal)
                {
                    ["object"] = ProjectObject(found, fields)
                });
        }
        ...
        public CapabilityInvocationResult QueryObjects(IReadOnlyDictionary<string, object?> request)
        {
            ...
            var limitResult = ReadLimit(request, MaximumPageSize, DefaultPageSize, out var limit);
            if (limitResult != null)
            {
                return limitResult;
            }
            ...
            var filterResult = ReadObjectFilter(request, out var filter);
            if (filterResult != null)
            {
                return filterResult;
            }

            var fingerprint = FingerprintObjectQuery(filter, fields, limit);
            var cursorResult = ReadCursor(snapshot, request, WorldInspectionContract.ObjectQueryName, fingerprint, out var offset);
        ...
        public CapabilityInvocationResult QueryReferences(IReadOnlyDictionary<string, object?> request)
        {
            ...
            var filterResult = ReadReferenceFilter(request, out var filter);
            if (filterResult != null)
            {
                return filterResult;
            }

            var fingerprint = FingerprintReferenceQuery(filter, limit);
            var cursorResult = ReadCursor(snapshot, request, WorldInspectionContract.ReferenceQueryName, fingerprint, out var offset);
            if (cursorResult != null)
            {
                return cursorResult;
            }
        ...
        public CapabilityInvocationResult QueryExtensions(IReadOnlyDictionary<string, object?> request)
        {
            ...
            var filterResult = ReadExtensionFilter(request, out var filter);
            if (filterResult != null)
            {
                return filterResult;
            }

            var fingerprint = FingerprintExtensionQuery(filter, limit);
            var cursorResult = ReadCursor(snapshot, request, WorldInspectionContract.ExtensionQueryName, fingerprint, out var offset);
            if (cursorResult != null)
            {
                return cursorResult;
            }
        ...
        public CapabilityInvocationResult ReadExtension(IReadOnlyDictionary<string, object?> request)
        {
            ...
            if (!TryGetInteger(request, "schemaVersion", out var schemaVersionLong) ||
                schemaVersionLong <= 0 || schemaVersionLong > int.MaxValue)
            {
            ...
            var limitResult = ReadLimit(request, MaximumExtensionChunkBytes, MaximumExtensionChunkBytes, out var limit);
            if (limitResult != null)
            {
                return limitResult;
            }
            ...
            for (var index = 0; index < snapshot.State.Extensions.Count; index++)
            {
                ...
                if (string.Equals(current.Owner, owner, StringComparison.Ordinal) &&
                    current.SchemaVersion == (int)schemaVersionLong)
                {
                    extension = current;
                    break;
                }
            ...
            if (extension == null)
            {
                return Failure(
                    ...
                    {
                        ["owner"] = owner,
                        ["schemaVersion"] = schemaVersionLong
                    },
                    false,
            ...
            var payloadBytes = extension.GetPayloadCopy();
            var count = Math.Min(limit, payloadBytes.Length - offset);
            ...
            if (offset + count < payloadBytes.Length)
            {
                data["nextOffset"] = offset + count;
            }
            ...
            for (var index = 0; index < values.Count; index++)
            {
                ...
                switch (value)
                {
                    ...
                    case "containerId":
                        ...
                        break;
                    case "references":
                        fields |= ObjectFields.References;
                        break;
                    default:
            ...
            out ObjectFilter filter)
        {
            ...
            var result = ReadTokenSet(data, "ids", "$.filter.ids", out var ids);
            if (result != null)
            {
                return result;
            }

            result = ReadTokenSet(data, "typeIds", "$.filter.typeIds", out var types);
            if (result != null)
            {
                return result;
            }

            result = ReadTokenSet(data, "containerIds", "$.filter.containerIds", out var containers);
            if (result != null)
            {
                return result;
            }

            result = ReadTokenSet(data, "referenceKinds", "$.filter.referenceKinds", out var kinds);
            if (result != null)
            {
                return result;
            }

            result = ReadTokenSet(data, "targetIds", "$.filter.targetIds", out var targets);
            if (result != null)
            {
                return result;
            }
            ...
            out ReferenceFilter filter)
        {
            ...
            var result = ReadTokenSet(data, "sourceIds", "$.filter.sourceIds", out var sources);
            if (result != null)
            {
                return result;
            }

            result = ReadTokenSet(data, "kinds", "$.filter.kinds", out var kinds);
            if (result != null)
            {
                return result;
            }
            ...
            out ExtensionFilter filter)
        {
            ...
            var result = ReadTokenSet(data, "owners", "$.filter.owners", out var owners);
            if (result != null)
            {
                return result;
            }
        ...
        private static IReadOnlyDictionary<string, object?> ProjectObject(WorldObject value, ObjectFields fields)
        {
            ...
            if ((fields & ObjectFields.References) != 0)
            {
                var references = new List<WorldReference>(value.References);
                ...
                for (var index = 0; index < references.Count; index++)
                {
                    items.Add(ReadOnly(new Dictionary<string, object?>(StringComparer.Ordinal)
                    {
                        ["kind"] = references[index].Kind.Value,
                        ["targetId"] = references[index].TargetId.Value
                    }));
                }

                data["references"] = items.AsReadOnly();
            }
        ...
        {
            payload["world"] = WorldMetadata(snapshot);
            return CapabilityInvocationResult.Succeeded(ReadOnly(payload));
        }

        private static IReadOnlyDictionary<string, object?> WorldMetadata(WorldSnapshot snapshot)
        {
        ...
        private static string FingerprintObjectQuery(ObjectFilter filter, ObjectFields fields, int limit)
        {
        ...
        private static string FingerprintReferenceQuery(ReferenceFilter filter, int limit)
        {
            return HashFingerprint("reference\n" + filter.Fingerprint() + "\nlimit=" + limit.ToString(CultureInfo.InvariantCulture));
        }

        private static string FingerprintExtensionQuery(ExtensionFilter filter, int limit)
        {
            return HashFingerprint("extension\n" + filter.Fingerprint() + "\nlimit=" + limit.ToString(CultureInfo.InvariantCulture));
        }
        ...
        private static bool TryConvertInteger(object? raw, out long value)
        {
            ...
            switch (raw)
            {
                ...
                case long current:
                    value = current;
                    return true;
                case ulong current when current <= long.MaxValue:
                    value = (long)current;
                    return true;
                default:
                    return false;
        ...
        private sealed class ObjectFilter
        {
            ...
            {
                if (Ids != null && !Ids.Contains(value.Id.Value))
                {
                    return false;
                }

                if (Types != null && !Types.Contains(value.TypeId.Value))
                {
                    return false;
                }

                if (Containers != null && (!value.ContainerId.HasValue || !Containers.Contains(value.ContainerId.Value.Value)))
                {
                    return false;
                }

                if (ReferenceKinds == null && Targets == null)
                {
                    return true;
                }
    ...
    /// <summary>Stable schema and definition source for HK03 world reads.</summary>
    public static class WorldInspectionContract
    {
        ...
        private static JsonSchemaDocument ReferenceQuerySuccessSchema()
        {
            ...
            var properties = WithWorldMetadata(new Dictionary<string, SchemaNode>(StringComparer.Ordinal)
            {
                ["items"] = SchemaNode.Array(reference),
                ["nextCursor"] = SchemaNode.String()
            });
            return ObjectSchema(properties, new[] { "world", "items" });
        }
        ...
        {
            var extension = SchemaNode.Object(
                new Dictionary<string, SchemaNode>(StringComparer.Ordinal)
                {
            ...
            var properties = WithWorldMetadata(new Dictionary<string, SchemaNode>(StringComparer.Ordinal)
            {
                ["items"] = SchemaNode.Array(extension),
                ["nextCursor"] = SchemaNode.String()
            });
            return ObjectSchema(properties, new[] { "world", "items" });
        }
        ...
        {
            return SchemaNode.Array(SchemaNode.String(new[] { "typeId", "containerId", "references" }));
        }

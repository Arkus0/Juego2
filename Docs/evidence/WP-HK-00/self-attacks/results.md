...
| `auto-directory-build-target` | `HK00-MSBUILD-FILE-UNCLASSIFIED` | RED on injection → pristine reconstruction → GREEN |
| `solution-noncsharp-participant` | `HK00-SOLUTION-UNEXPECTED-PROJECT` | RED on injection → pristine reconstruction → GREEN |
| `unclassified-msbuild-project` | `HK00-MSBUILD-FILE-UNCLASSIFIED` | RED on injection → pristine reconstruction → GREEN |
| `proof-project-inline-target` | `HK00-BUILD-XML-SHAPE` | RED on injection → pristine reconstruction → GREEN |
| `repository-response-file` | `HK00-BUILD-RESPONSE-FILE` | RED on injection → pristine reconstruction → GREEN |
| `alternate-solution-entrypoint` | `HK00-ALT-SOLUTION-ENTRYPOINT` | RED on injection → pristine reconstruction → GREEN |
| `unclassified-compiler-input-item` | `HK00-BUILD-XML-SHAPE` | RED on injection → pristine reconstruction → GREEN |
| `late-compiler-input-channel` | `HK00-COMPILER-INPUT-UNCLASSIFIED` | RED on injection → pristine reconstruction → GREEN |
| `proof-effective-generated-source` | `HK00-COMPILER-SOURCE-UNTRACKED` | RED on injection → pristine reconstruction → GREEN |
| `bootstrap-recursive-proof-source` | `DIRECT-CSC-BOOTSTRAP` | RED on injection → pristine reconstruction → GREEN |
| `postcompile-output-substitution` | `HK00-PDB-PE-MISMATCH` | RED on injection → pristine reconstruction → GREEN |
| `test-analyzer-outside-trusted-roots` | `HK00-COMPILER-ANALYZER-UNTRUSTED` | RED on late test analyzer injection → pristine reconstruction → GREEN |
| `dependency-lock-drift` | `NUGET_LOCKED_RESTORE` | RED on injection → pristine reconstruction → GREEN |
| `external-build-import` | `HK00-MSBUILD-IMPORT-EXTERNAL-UNTRUSTED` | RED on injection → pristine reconstruction → GREEN |
| `rogue-compiler-reference` | `HK00-COMPILER-REFERENCE-UNTRUSTED` | RED on compiler reference outside packs/dependency outputs/locked test packages → pristine reconstruction → GREEN |

...
The GATE readiness universe is fixed by `WP-HK-GATE` and by `HkGateReadinessTests.GateStepUniverseIsExplicitAndCannotSilentlyOmitARequiredScenarioStage`; it cannot shrink merely because an implementation/projection omits a capability.
...
| 14. full headless validation | full `Arkus.Harness.Tests` suite on candidate observation | GREEN |
| 15. causal false-green controls | one GATE stage-omission RED + twelve accepted HK10 causal RED controls in disposable worktrees | GREEN |
| 16. dependency/IP inventory + H0→Engine Bridge audit | `DEPENDENCY_IP_INVENTORY.md` | GREEN |
...
The representative GATE client likewise requires a fixed public capability set that includes discovery, object/reference/extension inspection, validation, mutation, journal v1/v2, snapshot, diff and replay. The 14-stage gate-step universe has its own omission RED control.
...
The deterministic Worker evidence cannot substitute for the mandated independent second client because this Worker has read implementation source and authored the candidate. A fresh agent trial must therefore remain a real external proof obligation. Until it posts a qualifying exact-SHA PASS transcript, the correct foundational verdict is `NOT_READY`.

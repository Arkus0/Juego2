...
Implementation observation SHA: `38783856d89a9ec7cff376086ae8702b0bb72501`
...
| Request/success/error schemas are machine-readable and one-source | Every accepted definition requires all three schemas; projection is generated directly from canonical definitions and its emitted portable data is checked by an independent expected-data oracle. | canonical positive tests; emitted-artifact structural conformance | missing schema; projection omission; schema/metadata alteration; optional-field omission | GREEN |
...
| Compatibility/version rules are explicit | Same-version semantic drift is decided by exact structural equality; same-major minor increments may be compatible/additive only under conservative rules; major changes are breaking. Runtime selects highest accepted compatible version. | compatibility + negotiation tests | semantic drift without version bump; delimiter-bearing metadata/schema collision | GREEN |
| Dispatcher surface equals canonical discovered/schema surface | Definitions are compared with composed route keys; every definition is schema-validated; conformance dispatches `system.describe` and compares the returned portable artifact against an independent structural projection oracle. | `CanonicalContractConformance` positive tests | dispatcher/binding mismatch; projection omission/alteration; optional semantic-field omission | GREEN |
...
| Projection cannot silently omit or alter canonical semantics | Runtime conformance evaluates the actual `system.describe` result. A separate model-to-portable-data oracle does not call the production `ToData()` projectors, and semantic/compatibility decisions use structural equality rather than fingerprints. | emitted-artifact conformance positive path | omission, extra/altered schema or metadata semantics, optional field omission, delimiter-bearing collisions | GREEN |
| Canonical Linux build/test path remains healthy | HK01 canonical observation uses locked restore, Release build and full regression on exact clean SHA. | Actions run `35434057309`: 0 warnings/errors; 9/9 canonical positives; 22/22 causal negative controls; 32/32 total tests | exact-SHA/clean-worktree gates in observation script | GREEN |

## Proof-budget conclusion

The proof surface maps directly to HK01 acceptance and required self-attacks. The provider-filtered route universe was removed rather than patched with more filter cases. Repair cycle 1 likewise replaced a collision-prone fingerprint correctness oracle with structural equality and bound conformance to the emitted discovery artifact through an independent expected-data oracle. The additional machinery closes the exact projection/compatibility false-green class observed by independent review; it does not expand into arbitrary toolchain hostility.

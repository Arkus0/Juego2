...
Current failure lineage: `fail_cycle: 4`
...
After three Reviewer FAILs in the same false-green family, the boundary was re-audited before cycle-3 repair. Cycle 4 keeps that same boundary: deterministic production facts, bounded external test oracles for selected representative semantics, and escalation for everything else that needs natural-language judgment. The fourth FAIL exposed two missing causal controls inside that already chosen split, not a need to widen the architecture.
...
- accepted PA-chain universe from COMPLETE canonical PA workpacks, with each required canonical result and capsule checked afterward rather than allowing result files to define the inventory;
...
The causal controls preserve ID, source pointer and source fingerprints while inverting/inventing statement text, and the representative oracle REDs. Cycle 4 additionally removes one of many exports and one of many exclusions from the actual indexed `WP-HK-GATE` capsule: production shape/source validation deliberately remains GREEN, while the same representative semantic oracle must RED. This proves semantic loss detection rather than merely proving that a test mutation deleted an ID.
...
Fingerprints are only useful when the chosen source and the universe being checked are independently constrained. The repaired design therefore requires:
...
- COMPLETE `WP-PA-NN.md` workpacks to define the PA-chain universe before requiring the corresponding canonical `PA-NN.md` result and capsule;
- accepted PA `disposition_source` to be that exact completion-derived canonical result;
...
- PA: accepted result capsules for `WP-PA-01`, `WP-PA-02`, `WP-PA-03`. The representative PA consumer inherits the full three-capsule chain; semantic fixtures therefore cover all three current capsules, while PA-03 additionally exercises directional semantics. Future COMPLETE PA workpacks automatically enter structural chain completeness, but arbitrary future prose remains escalation/human territory unless deliberately added to a later bounded representative semantic test.
...
The disposition table is a production oracle only because its source identity is independently anchored to the canonical result derived from a COMPLETE workpack inventory.
...
The cycle-4 repair is acceptable only while it stays inside this split:

- small deterministic production invariants;
- bounded test-only representative semantic fixtures for the five actual current consumer capsules, including explicit one-of-many omission defects, not an open-ended production registry;

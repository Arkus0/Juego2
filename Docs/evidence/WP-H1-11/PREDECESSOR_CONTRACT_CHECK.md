...
Three later hosted findings were classified the same way (details in `RESIDUAL_RISK.md`):

3. **Capture exit-time write (F8).** The supplementary capture's Editor process left drawn source materials dirty and the Editor's exit save rewrote them; the accepted H1-10 source fingerprint caught it (`checkpoint.source-rebound`). The inherited guarantee held; the cause was H1-11's own capture stage, fixed there.
4. **Removed asset-node code (F9).** For a removed source used by an asset node, the accepted H1-09 observer names the node with `projection.source-wrong-type` rather than `source-missing`. H1-09's guarantee (the drifted node is named) holds; only the code granularity is coarse, and the missing logical ID is named by the live catalogue and the checkpoint. No reopen.

5. **Per-import observation (F10).** The normalized observation carries the GUIDs of importer-generated nested materials, which each clean import recreates, so checkpoint/reconstruction digests are per import. H1-10's accepted parity (clean generated-output rebuild) held in every run; cross-import portability is its named non-claim and H1-04 keeps those nested materials outside the catalogue universe. No reopen.

...
| Accepted predecessor residual sections are completely inventoried | test extracts the explicit bounded/non-blocking residual sections from accepted DW-00..03 `RESIDUAL_RISK.md` evidence and requires exact anchor-set equality; DW-04 absence of a residual file plus its accepted diagnostic-only token/cost bound is checked explicitly | `Dw05ResidualInventoryCompletenessTests` |
| Every inventoried predecessor residual is reconciled | independent source/anchor inventory is verified against source bytes and every limitation id must resolve to exactly one manifest entry | `PredecessorResidualManifestReconcilesIndependentEvidenceInventory` |
| Residual omission fails | complete independent inventory remains fixed while manifest clone drops one id | `ResidualReconciliationRedsForOmissionAndUnsupportedKernelClassification` |

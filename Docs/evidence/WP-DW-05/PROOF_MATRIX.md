...
| Shared generic/H0 source is CITY/PA-neutral | finite audited generic files + entire top-level `src/Arkus.Game.World` source/project set | `GenericDwAndH0DependencyClosureContainsNoCityOrPaLeakageAndInjectedLeakageReds`; injected `CityKernelRule` must RED |
| DW-05 did not silently patch accepted generic/H0 semantics | baseline Git blob identities + baseline→candidate canonical diff | `AcceptedGenericDwAndH0SeamBytesRemainAtBaselineBlobIdentities` plus `scripts/dw05-observe-exact-sha.sh` |
...
The proof attacks the material causal boundaries owned by DW-05: neutral representability, generic provenance/rebuild, domain leakage, silent H0 mutation, residual completeness/classification and H2 overclaim. It does not expand into arbitrary compiler/toolchain corruption or future unclaimed domains.

...
- public H0 reference query via `WorldInspectionService`, correctly bound to the current public revision/hash contract;
...
- empty fact-level diff after identical rebuild;
...
| H0 object/reference queries require the accepted revision+content-hash anchor rather than an empty request. | in-scope integration finding, repaired | DW uses the public stale-read guard as designed instead of weakening/bypassing it. |

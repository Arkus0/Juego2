...
- **Allocation relation oracle:** `Dw01InteriorRelationOracleTests` independently parses the accepted CITY-02 I1–I3 subject set and inspects the already projected `DesignFact.Relations`; for every subject it requires exactly one literal `allocates-depth` relation from `allocation.<id>` to `depth.<id>`. This oracle does not consume `AnchoredFactDefinition`, so changing/removing/redirection of the projection rule cannot self-confirm through `DesignWorldProjectionValidator`.
...
| Second invariant exercises a different structural relation/cardinality shape | DW relation coverage | an oracle separate from the projection definitions derives the 14 I1–I3 subjects directly from CITY-02 and then requires exactly one `allocation.<id> --allocates-depth--> depth.<id>` edge in the projected facts | accepted real CITY projection is checked for all 14 allocation edges | three synthetic projections keep the same facts/universe and remain GREEN under the generic validator while respectively removing `allocates-depth`, renaming it, or redirecting it to another in-universe depth fact; the independent CITY oracle makes each case RED | GREEN by independent positive + causal negative tests | Broader CITY graph relations move to DW-02+. |
...
8. **Missing allocation relation:** projected allocation keeps its fact/fields/universe but loses `allocates-depth`; the generic projection validator self-confirms and the independent CITY relation oracle REDs.
9. **Renamed allocation relation:** projected allocation keeps the same target but changes the relation type; the independent CITY relation oracle REDs because the required literal relation is absent.
10. **Wrong allocation relation target:** projected allocation keeps one `allocates-depth` edge but points to another valid in-universe depth fact; the generic validator remains GREEN and the independent CITY oracle REDs.
...
13. **Validator-rule execution removed:** the mutation tests require `BuildAndValidate` itself to throw the named semantic error for the runtime semantic checks; deleting/bypassing either validation pass makes those tests fail rather than allowing a false GREEN. Relation-shape coverage is separately protected by the independent projection oracle above.

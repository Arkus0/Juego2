...
- **Allocation relation oracle:** production `CityInteriorRelationOracle` is invoked by `CityDesignWorldProvider.BuildAndValidate` after generic fresh-projection validation. `BuildAndValidate` supplies the I1–I3 obligation set derived from accepted CITY-02, while the oracle inspects only the already projected `DesignFact.Relations`; for every required subject it requires `allocation.<id>` and `depth.<id>` to exist and exactly one literal `allocates-depth` edge to target that subject's own depth fact. The oracle does not consume `AnchoredFactDefinition`, the authority reader used to build the projection, or any relation registry. Its `CityInvariantException` diagnostics carry a stable machine code, `SubjectId`, explicit semantic rule, expected/observed relation detail and CITY-02/CITY-06 source paths.
...
16. **Validator-rule execution removed:** the runtime semantic checks are called by `BuildAndValidate`; bypassing access/depth/relation validation makes their positive/negative proof obligations fail rather than allowing the claimed pipeline to remain falsely GREEN.

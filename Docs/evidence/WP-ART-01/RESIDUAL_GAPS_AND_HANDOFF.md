...
| URP rendition if the consumer Unity project changes pipeline | A future bridge/art adapter must explicitly remap the built-in Standard materials. The current H1 Unity project is built-in, as audited. |
...
The final production handoff is: locked source/adoption and derivative ledger; `KIT_COMPOSITION_MANIFEST.json` with hosts/dimensions/readiness; `ASSEMBLY_PLANS.md` and the accepted dimensional profile; frozen visual targets; local Unity benchmark scene, generated ART mesh/material assets and third-person/neutral captures; fresh-author smoke-test result; and the separate #233 lesson/boundary record. CITY-07 can use the vocabulary without treating this local benchmark as canonical town geometry.

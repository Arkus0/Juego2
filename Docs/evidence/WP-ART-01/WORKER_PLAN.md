...
Baseline `main`: `648272bb50a0c5c718a9f0f4668faaaa0482aef9`. Canonical branch: `codex/wp-art-01`. Predecessor ownership was reconstructed **before implementation** in `PREDECESSOR_CONTRACT_CHECK.md`. PR #233 is frozen non-canonical diagnostic evidence; no file under `Unity/PrototypeDemo` is edited or imported as a scene/seed.

## Claim and evaluation

The final candidate must supply a bounded, provenance-safe Juego2 visual vocabulary and assembly grammar, demonstrated in an actual Unity third-person building/street/threshold benchmark on the retained chain. Every mandatory benchmark element must be `KEEPER_READY`. Positive evidence is the target sheet, demand disposition, measured source/derivative kit metadata, assembled scene and three meaningful checkpoints, normal/neutral human-scale captures and fresh-author assembly revision. Negative gates include decorated cuboids, sticker openings, floating roof, unresolved contact, duplicated collision, unapproved source, raw unclothed scale mannequins and silent `PROXY_VISUAL`/`COVERAGE_BLOCKED` success. No CITY topology, H1 semantics, H2 compiler, NPC behavior or whole-town keeper is claimed.

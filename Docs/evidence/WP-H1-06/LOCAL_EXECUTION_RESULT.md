...
## Multiplicity repair observation

The first physical-local run on repair SHA `c4425dcc93e79fb536d1937bca6cc383a45b07f8` reached the updated Unity fixture and produced its new `GREEN` result, but the canonical PowerShell script rejected it because its gate still expected the pre-repair `productPath` and `negative` strings. That is a real integration failure; it is not a GREEN receipt for that SHA. The Worker updated the script to require the repaired fixture's schema, two-sibling positive, product observation/digest path and one-sibling-loss negative.

The complete command then ran GREEN on clean SHA `e1e34da7d9625309bcdcd8c09d6f4296dcd84c73` with the pinned physical Unity `6000.3.24f1 (4e7b9b5b6244)`, .NET `8.0.425`, project `arkus.unity-project@1:ArkusUnity`, the unchanged package manifest/lock fingerprints above, and the approved Quaternius Source archive fingerprint. Unity reported `two-same-name-nested-prefab-links-preserved-after-save-reload`, `ObserveRealization:nested-prefab-multiplicity-and-digest`, and `one-of-two-same-name-siblings-removed:projection.prefab-nested-lineage-missing`; the fixture also exercised the flattened-child control. The content-shape probe found wall, roof, door, window and prop candidates. Locked Release build succeeded, the three focused .NET tests passed, and public conformance returned four realized Quaternius nodes, stable missing/wrong-type/rebound diagnostics, unchanged source and canonical hash/journal, same-input idempotence, and derivative delete/rebuild relationship convergence. Initial realization digest: `556dd9d753686a86e3709780c83c3bcbe59ac0c05fc0ffe66ac383bfdb532a4b`. The candidate remained clean before and after.

This evidence update changes the SHA. The final candidate still requires the complete read-only command and durable exact-SHA PR receipt before freeze.

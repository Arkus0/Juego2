...
→ causal self-attacks / negative controls proportional to the claim
...
Unless a workpack explicitly narrows it further or has a product/security requirement that justifies expanding the proof boundary, foundational work may trust:
...
The repository must pin/version/configure these layers where reproducibility materially depends on doing so. Proof may verify that the intended pinned/configured toolchain is actually selected and that the repository uses the canonical path, but it does **not** need to prove that the trusted toolchain itself cannot be maliciously subverted by arbitrary hostile repository tricks.

A workpack may intentionally move part of this trusted base inside its claim only when its acceptance criteria genuinely require that guarantee, for example sandbox/security work. The WP must then state that expansion explicitly and justify its cost.
...
Attacks should target causal omission classes, not syntax variants. If multiple bypasses share one causal class, fix/prove the class rather than accumulating case-specific checks.

Do not add a new self-attack merely because a hypothetical hostile repository can exploit undocumented or intentionally out-of-bound behaviour of the trusted base. Record such cases as non-blocking residual risk unless the WP explicitly owns that guarantee.
...
Every material guard, oracle or causal attack added after the initial implementation must map to at least one of:
...
Do not add proof infrastructure solely to reduce philosophical residual risk or to demonstrate arbitrary hostility against trusted infrastructure.
...
- a newly imagined attack outside the trust boundary is not by itself justification for another repair cycle.
...
Risks that require corruption/subversion of the declared trusted base, use a non-canonical unsupported build path, or concern guarantees the WP does not claim must be documented when useful but are non-blocking by default.
...
- a material in-boundary self-attack remains unexercised;
...
Do not block freeze solely because an out-of-boundary trusted component could theoretically be subverted.
...
The Reviewer is not required to invent a new defect class in order to PASS, and must not fail a candidate merely by demonstrating arbitrary subversion of infrastructure explicitly declared trusted unless the WP acceptance criteria put that infrastructure inside the claim.

...
| `R-05-06` | HK-05, HK-08A | Whole-world size, latency and final agent interaction budgets are not product guarantees | HK-09B for content-size caps; HK-08B for measured latency/interaction baselines | `DEFERRED` |
...
| `R-06A-05` | HK-06A, HK-07A, HK-08A | Session-local provenance journal growth remains unbounded internally; HK08A bounds the explicit v2 public read but does not claim incremental storage/indexing or long-session growth control | HK-10 for bounded-session growth evidence inside the HK-09B envelope | `DEFERRED` — HK08A PASS `#5260092080` closed the bounded-read/pagination half with explicit `authoring.journal.read@2.0`; growth remains open to the named owner |
...
| `R-07A-01` | HK-07A, HK-07B, HK-08A | Admission-only cancellation/timeout and current transport/interaction framing bounds are not general post-dispatch execution-time, CPU/memory, request/output-size or resource-governance guarantees | HK-09B | `DEFERRED` |
| `R-07A-02` | HK-07A | `--file PATH` accepts an explicit caller path and HK07A does not claim filesystem sandbox/allow-list, path-containment or broader host-capability policy | HK-09A | `DEFERRED` |
| `R-07A-03` | HK-07A, HK-07B, HK-08A | Structured stale-revision recovery, repair ergonomics and measured end-to-end interaction budgets remain outside the accepted interaction primitives | HK-08B | `DEFERRED` — HK08A PASS `#5260092080` closed batching, compact reads and pagination; HK08B owns the remaining stale-CAS recovery/repair/benchmark claim |
...
| `R-03-02` | HK-03, HK-08A | Cursors are deterministic/integrity-framed continuation tokens, not authenticated capabilities, and are not an authorization boundary | HK08A detects altered v2 journal continuation context but explicitly does not claim a secret MAC/auth boundary; HK09A constrains host authority without turning cursors into authentication capabilities |
...
| `R-04-01` | HK-04, HK-06A, HK-06B, HK-06C, HK-07A, HK-07B, HK-08A | Canonical state, mutation idempotency receipts/journal, snapshots, snapshot-rebase receipts/evidence, replay staging and the accepted JSONL/MCP production hosts remain process-local; HK08A pagination slices the accepted in-memory journal and adds no durable WAL/recovery or cross-process persistence claim | fresh-process reconstruction remains explicit snapshot import/journal replay; automatic crash/restart recovery remains outside the accepted claim |
| `R-04-02` | HK-04, HK-06C, HK-07A, HK-07B | Multi-process/distributed writers, cross-process locking and multi-agent merge/coordination | HK07A/HK07B remain process-local projections over the accepted whole-world CAS and make no cross-process coordination claim; the post-GATE H0S track remains the default measurement venue, not a binding owner |
...
| `R-07A-04` | HK-07A, HK-07B | Authentication, remote tenancy, encrypted/network transport semantics and hostile OS/process isolation are not claimed by the local stdin/stdout/stdio hosts | HK09A explicitly excludes authentication/multi-user cloud security and OS sandboxing; no accepted H0 owner is named for these external/remote concerns |
...
Reading it: the cost is per **transaction**, not per edited object, because identity is whole-world. A batched authoring flow pays it once; an unbatched one pays it per edit. `WP-HK-08A` now supplies the accepted representative batching primitive, while `WP-HK-08B` owns the measured end-to-end interaction baseline.
...
| In-memory lifetime and durability | `R-04-01` | HK-04, HK-06A, HK-06B, HK-06C, HK-07A, HK-07B, HK-08A |
| Undeclared identity inside opaque payloads | `R-02A-01` | HK-02A, HK-05 |
| Size, byte, latency and resource budgets | `R-02A-04`, `R-04-04`, `R-05-06`, `R-06A-05`, `R-07A-01` | HK-02A, HK-04, HK-05, HK-06A, HK-07A, HK-07B, HK-08A |
| Large-world scaling with no owner | `R-03-01`, `R-04-05`, `R-06B-06` | HK-03, HK-04, HK-06B, HK-06C, HK-07A, HK-08A |
...
| Cross-process/concurrent writers | `R-04-02`, `R-02A-02` | HK-04, HK-06C, HK-07A, HK-07B, HK-02A |

The budget theme now uses three different mechanisms. HK-08A has accepted **interaction primitives** such as representative batching and bounded v2 journal/page reads. HK-08B owns **measured end-to-end baselines and regression budgets**. HK-09B owns **enforced caps** such as input/batch/page/depth/resource limits that fail closed. Journal/session growth itself is exercised by HK10 against the accepted HK09B resource envelope.

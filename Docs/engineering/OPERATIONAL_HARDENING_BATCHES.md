...
- `scripts/validate-worker-preflight-contract.py` checks the local/delegated wiring across the workflow, local script, implementation/repair skills, canonical protocol and this plan, including negative mutations that remove exact-SHA checkout, live-head verification, delegated fallback or grandfathering.

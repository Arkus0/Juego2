...
- Worker preflight with two equivalent execution paths for post-adoption cycles. Prefer `scripts/worker-preflight.sh` on the clean exact candidate HEAD when the Worker environment can execute the exact SDK from `global.json`; a valid local result ends `WORKER_PREFLIGHT_GREEN`. If that environment lacks the exact SDK, use the repository-owned `Worker Candidate Preflight` GitHub Actions run for the canonical PR and exact candidate SHA; a valid delegated receipt ends `WORKER_PREFLIGHT_DELEGATED_GREEN`. The delegated run checks out `pull_request.head.sha`, installs the exact SDK, runs locked restore + Release build/tests + process self-tests, rechecks the live PR head before emitting a durable `PR + SHA + run_id` receipt, and is cancelled/replaced when the PR advances. A local capability miss is therefore not `NOT_READY` by itself; a missing/mismatched/stale delegated receipt is.
...
- `Arkus Main Safety` build/test/process safety workflow on pushes to `main` and on every pull request. Pull-request execution is intentionally unfiltered: MSBuild imports, analyzers, generators, project metadata and future project configuration can make repository files material to restore/build/test even when their paths are not known in advance. `scripts/validate-main-safety-trigger.py` is a deliberately small regression guard that checks the `pull_request` trigger remains present and unfiltered; it does not model general GitHub Actions execution semantics.
...
- `scripts/validate-worker-preflight-contract.py` checks the expected local/delegated wiring across the workflow, local script, implementation/repair skills, canonical protocol and this plan, including exact-SHA checkout, delegated fallback, locked-restore command text and grandfathering. It is intentionally a shallow regression guard, not a Bash/YAML interpreter, and does not claim to prove reachability against deliberately deceptive script/workflow edits.
...
No persistent workstation dependency is introduced. Workers that already have the exact SDK use the cheaper local path; chat/cloud Workers without it consume the exact-context delegated run instead. Locked restore is required on Worker preflight and Main Safety because the repository already commits the package lockfiles consumed by the solution. Branch protection is enabled only after Batch B supplies the stable merge-gate check.

Hardening freezes after the #147 follow-up. The retained guards are scoped to accidental configuration/wiring regressions and the normal review trust boundary. They are not intended to become adversarial interpreters of Bash or GitHub Actions, and deliberately deceptive edits to the guard surfaces remain a Reviewer concern rather than a new static-analysis layer.

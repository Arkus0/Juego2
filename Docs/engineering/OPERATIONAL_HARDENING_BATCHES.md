...
- `Arkus Main Safety` build/test/process safety workflow on pushes to `main` and on every pull request. Pull-request execution is intentionally unfiltered: MSBuild imports, analyzers, generators, project metadata and future project configuration can make repository files material to restore/build/test even when their paths are not known in advance. `scripts/validate-main-safety-trigger.py` rejects reintroduction of nested `pull_request` filters so this protection cannot silently shrink back to an incomplete allow-list.
...
- `scripts/validate-worker-preflight-contract.py` checks the local/delegated wiring across the workflow, local script, implementation/repair skills, canonical protocol and this plan, including negative mutations that remove exact-SHA checkout, live-head verification, delegated fallback, executable locked-restore parity or grandfathering. Locked restore must exist as an actual executed command; preserving its literal only in comments, `echo` output or other non-executed text remains RED.

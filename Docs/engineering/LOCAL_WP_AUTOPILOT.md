...
`PROTOCOL_FIX` is not a material FAIL and does not increment the FAIL count. The controller may launch one bounded fresh metadata-only corrector for the unchanged `PRODUCT_SHA`, then wait for a new context-bound `REVIEW_READY` without rerunning the product suite. A repeated or interrupted correction, SHA movement, or `REVIEW_BLOCKED` stops for PC. The subsequent Reviewer session must reuse trustworthy exact-SHA mechanical receipts and focus its independent judgment on the material claim; it must not treat the protocol correction as a new product cycle.

...
```powershell
python scripts/local_wp_autopilot.py --root . --next --dry-run
python scripts/local_wp_autopilot.py --root . --wp H1-03 --one-wp
python scripts/local_wp_autopilot.py --root . --next
```

The first two commands are the recommended pilot **once H1-03 is dependency-valid**: inspect the read-only route, then complete one explicit WP with `--one-wp`. The normal third command continues automatically after each merged `DOCSYNC_COMPLETE` names a dependency-valid next WP and quota permits. `--adopt` is an explicit recovery switch for an existing open PR **only after its current Worker/Reviewer has stopped**; it does not relax one-active-Worker ownership. A dirty worktree, ambiguous WP/PR, missing marker, invalid SHA, missing auth/quota data, unavailable model/effort, or local CLI/GitHub failure stops the driver and requests attention. The default local state directory takes an OS-released single-process lock; using a different `--state` is not a safe way to start another owner of the same worktree.

`--next` follows the most recent **owner-authored** live merged `DOCSYNC_COMPLETE` marker only after checking its PR/WP/key, post-merge timestamp, reconciled commit's descent from that PR's merge and ancestry on current main, WP-specific evidence/index diff, and persisted first-parent projection. If no such handoff exists or it points to an already-owned PR, stop; use `--wp` only after verifying the roadmap/dependencies. The cheap precheck requires each named WP dependency to be merged and DocSync-evidenced on current main; non-WP prerequisites require explicit verification. This is not acceptance proof: the Worker still performs mandatory predecessor and live GitHub reconstruction and stops if the WP is blocked. The dedicated worktree is reset to a clean detached `origin/main` **only before a new WP**, never while an open candidate is being worked. Runtime logs and the lock live under `%LOCALAPPDATA%/Arkus/Juego2/autopilot`, not in the repository. Treat logs as local private data.
...
| Second-FAIL audit | GPT-6 Luna xhigh | Classifies concrete valid defect, overdefense or uncertainty; never emits/supersedes a verdict. |
...
3. Reuse a valid exact-SHA GREEN receipt for the **identical** candidate; never rerun expensive validation merely to move a state marker.
...
These rules do not compress away non-compressible proof sources, local Unity evidence, or the Reviewer's independent causal search.

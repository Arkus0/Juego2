...
The next CTX action is `WP-CTX-03 — Structured evidence, quality-preserving context envelope, DocSync and history separation`. It is dependency-valid after this CTX-02 DocSync and remains `NOT_STARTED` until a human explicitly starts its Worker. CTX does not semantically block H1/CITY/PA; DW-04 separately waits for accepted CTX-03 as its comparison baseline.

...
- If the expected base is not provably recoverable from the current local lineage (for example across an unrelated/new snapshot-rebase lineage or unavailable history), the harness does not fabricate a delta. It returns a stable machine-readable disposition explaining that bounded reinspection is required.

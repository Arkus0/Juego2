...
- **Batch A Worker-preflight adoption:** if this implementation cycle first enters `DRAFT + ACTIVE` after the operational-hardening Batch A PR has merged to `main`, exact-candidate Worker preflight evidence is mandatory immediately before the strict Worker pre-review. Prefer `scripts/worker-preflight.sh` on the clean exact candidate HEAD when this Worker environment can execute the exact SDK from `global.json`; record `Candidate SHA: <exact HEAD>` plus `WORKER_PREFLIGHT_GREEN`. If the environment lacks that exact SDK and the script reports `WORKER_PREFLIGHT_DELEGATION_REQUIRED`, use the repository-owned `Worker Candidate Preflight` GitHub Actions run for the canonical PR and the same exact candidate SHA. The delegated run is valid only when its durable receipt names `Arkus0/Juego2`, the canonical PR number, that exact SHA and its run ID, ends `WORKER_PREFLIGHT_DELEGATED_GREEN`, and the live PR HEAD still equals that SHA. A result from another PR/repository/SHA or an older candidate is invalid. Missing both a valid local result and a valid delegated receipt is `NOT_READY`; lack of local .NET alone is not. A cycle already active, frozen or in review before adoption is grandfathered and is not reopened solely for this check. If a later fresh repair cycle starts after adoption, that repair cycle is governed.
...
13. Stop writers and read the exact 40-character HEAD that will be challenged. For cycles governed by Batch A, obtain exact-candidate Worker-preflight GREEN now: run `scripts/worker-preflight.sh` against that clean HEAD when the exact SDK is executable here; otherwise verify the `Worker Candidate Preflight` pull-request run for this canonical PR has a durable `WORKER_PREFLIGHT_DELEGATED_GREEN` receipt bound to this same SHA and run ID, and re-read the live PR HEAD after the run. Preserve the local output or delegated run/receipt link in pre-review evidence. Then perform the mandatory strict Worker pre-review against that exact resulting HEAD:
...
14. If pre-review finds an in-claim blocker, resume Draft + ACTIVE, repair the causal defect boundary, rerun affected validation/evidence, repeat the required local round when the repair can affect local evidence, commit/push the changed bytes, then repeat steps 12–13 on the new exact HEAD. Any local Worker-preflight result and any delegated receipt for the old SHA are invalid for the new candidate; obtain a new exact-SHA result before pre-review.
...
Any implementation/evidence mutation after a clean pre-review invalidates that cleanliness and requires the complete pre-review/freeze sequence to be rerun on a new exact SHA. For cycles governed by Batch A, a new exact-SHA Worker-preflight result is also mandatory: local `WORKER_PREFLIGHT_GREEN` when the exact SDK is available, otherwise a new delegated `WORKER_PREFLIGHT_DELEGATED_GREEN` receipt for the canonical PR and new SHA. Metadata-only correction on the same SHA may reuse the exact-SHA CLEAN record and durable REVIEW_READY marker only when the corrected gates are rerun and closure re-observes the terminal invariant.

...
10. For H1-02..H1-GATE when effective local Unity/Windows execution remains necessary, do **not** hand the whole Worker role to the local agent. Persist `Docs/evidence/<WP-ID>/LOCAL_EXECUTION.md` with an exact input SHA, commands/actions, expected outputs, allowed mutation paths, stop conditions and result destination; then pause for `.agents/skills/local-execute-h1/SKILL.md` execution.
11. After local execution, reconstruct live GitHub state, verify the local input/result SHA binding and mutation allowlist, interpret the effective evidence, and perform any required design/code repair as the remote Worker. If a repair can invalidate local evidence, issue a new numbered local round rather than extrapolating stale evidence.

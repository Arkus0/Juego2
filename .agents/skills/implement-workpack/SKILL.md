...
- **Batch A Worker-preflight adoption:** if this implementation cycle first enters `DRAFT + ACTIVE` after the operational-hardening Batch A PR has merged to `main`, `scripts/worker-preflight.sh` is mandatory on the clean exact candidate HEAD immediately before the strict Worker pre-review. Record its `Candidate SHA: <exact HEAD>` and `WORKER_PREFLIGHT_GREEN` output in Worker pre-review evidence. A cycle already active, frozen or in review before adoption is grandfathered and is not reopened solely for this check. If a later fresh repair cycle starts after adoption, that repair cycle is governed. Missing exact SDK/tooling is `NOT_READY`; GitHub Actions may not substitute for this Worker-environment preflight.
...
13. Stop writers and read the exact 40-character HEAD that will be challenged. For cycles governed by Batch A Worker-preflight adoption, run `scripts/worker-preflight.sh` now against that clean exact HEAD and continue only if its exact-SHA result is GREEN; preserve the command/output in the pre-review evidence. Then perform the mandatory strict Worker pre-review against that exact resulting HEAD:
...
14. If pre-review finds an in-claim blocker, resume Draft + ACTIVE, repair the causal defect boundary, rerun affected validation/evidence, repeat the required local round when the repair can affect local evidence, commit/push the changed bytes, then repeat steps 12–13 on the new exact HEAD. The old cleanliness and any old Worker-preflight result are invalid for the new SHA.
...
Any implementation/evidence mutation after a clean pre-review invalidates that cleanliness and requires the complete pre-review/freeze sequence to be rerun on a new exact SHA. For cycles governed by Batch A, the exact-SHA Worker preflight must also be rerun on that new SHA. Metadata-only correction on the same SHA may reuse the exact-SHA CLEAN record and durable REVIEW_READY marker only when the corrected gates are rerun and closure re-observes the terminal invariant.

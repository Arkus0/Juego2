...
4. Run exact positive tests through the canonical validation entrypoint.
5. Build a completeness inventory for finite in-claim surfaces where required.
6. For foundational claims, self-attack material defect classes inside the declared trust boundary and prove causal RED→GREEN.
...
10. If pre-review finds an in-claim blocker, keep Draft + ACTIVE, repair the causal defect boundary, rerun affected validation/evidence, and repeat step 9.
11. Record `WORKER_PRE_REVIEW: CLEAN`, findings fixed count and evidence path only when no known in-claim blocker remains. `CLEAN` is Worker readiness evidence, never independent `PASS`.
12. Stop all writers.

...
5. after CTX-02 adoption, run `python3 scripts/context-capsule-check.py --self-test` and `python3 scripts/context-capsule-check.py --audit-index --repo-root .` whenever capsule/index state is affected; a missing/invalid accepted PA capsule means capsule-chain coverage is incomplete, not that the authoritative PA result is invalid;

...
[[ -z "$(git status --porcelain --untracked-files=all)" ]] || { echo "Candidate is not clean before HK02A observation" >&2; exit 2; }
...
DOTNET_NOLOGO=1 dotnet test tests/Arkus.Harness.Tests/Arkus.Harness.Tests.csproj \
  ...
  --filter 'FullyQualifiedName~Hk02AObjectScopedExtensionTests'
DOTNET_NOLOGO=1 dotnet test tests/Arkus.Harness.Tests/Arkus.Harness.Tests.csproj \
  --configuration Release --no-build --no-restore

[[ -z "$(git status --porcelain --untracked-files=all)" ]] || { echo "Candidate is not clean after HK02A observation" >&2; exit 2; }
...
Canonical command: scripts/hk02a-observe-exact-sha.sh ${actual}
Candidate clean before: YES
Candidate clean after: YES
Required gates: locked-restore=GREEN; release-build=GREEN; object-extension-contract=GREEN; causal-negative-controls=GREEN; regression=GREEN
Result: GREEN

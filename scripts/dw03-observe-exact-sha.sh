...
BASELINE_SHA="e7d15a3a6ddcba51848b738fc6f33b69cf1fd8d1"
cd "${ROOT}"
...
git cat-file -e "${BASELINE_SHA}^{commit}" || { echo "DW-03 baseline commit is unavailable" >&2; exit 2; }
[[ -z "$(candidate_dirty_status)" ]] || { echo "Candidate is not clean before DW-03 observation" >&2; candidate_dirty_status >&2; exit 2; }
...
test -f src/Arkus.DesignWorld/PaCorpusProjection.cs
test -f src/Arkus.DesignWorld/PaSourceCorpusOracle.cs
test -f src/Arkus.DesignWorld/PaCorpusQueries.cs
test -f tests/Arkus.Harness.Tests/Dw03PaCorpusProjectionTests.cs
test -f Docs/evidence/WP-DW-03/PREDECESSOR_CONTRACT_CHECK.md
test -f Docs/evidence/WP-DW-03/WORKER_PLAN.md
test -f Docs/evidence/WP-DW-03/SOURCE_MANIFEST_V1.md
test -f Docs/evidence/WP-DW-03/QUERY_SUITE_V1.md
...
DOTNET_NOLOGO=1 dotnet test tests/Arkus.Harness.Tests/Arkus.Harness.Tests.csproj \
  ...
  --filter 'FullyQualifiedName~Dw03'

# DW-03 is a downstream PA consumer. No PA-corpus vocabulary or Design World
# dependency may leak back into accepted H0/public harness assemblies.
if grep -R -E 'Arkus\.DesignWorld|Pa(Corpus|DesignWorld|SourceCorpus)|pa\.semantic_|dw03-pa' \
  ...
  echo "DW-03 introduced a reverse H0/domain dependency on the Design World PA corpus consumer" >&2
  exit 2
fi
...
if grep -R -E '<ProjectReference' src/Arkus.DesignWorld/Arkus.DesignWorld.csproj | \
  ...
  echo "DW-03 added an unexpected project dependency to Arkus.DesignWorld" >&2
  exit 2
fi

grep -Fq '../Arkus.Game.World/Arkus.Game.World.csproj' src/Arkus.DesignWorld/Arkus.DesignWorld.csproj

# Accepted PA documents remain authority and are immutable within DW-03.
...
[[ -z "$(candidate_dirty_status)" ]] || { echo "Candidate is not clean after DW-03 observation" >&2; candidate_dirty_status >&2; exit 2; }
...
Canonical command: scripts/dw03-observe-exact-sha.sh ${actual}
Candidate clean before: YES
Candidate clean after: YES
Required gates: locked-restore=GREEN; release-build=GREEN; focused-dw03=GREEN; reverse-h0-dependency=GREEN; h0-domain-neutrality=GREEN; designworld-dependency-boundary=GREEN; pa-authority-immutability=GREEN; regression=GREEN
Result: GREEN

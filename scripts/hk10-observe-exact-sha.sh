...
[[ -z "$(git status --porcelain --untracked-files=all)" ]] || { echo "Candidate is not clean before HK10 observation" >&2; exit 2; }
...
[[ -z "$(git status --porcelain --untracked-files=all)" ]] || { echo "Candidate is not clean after HK10 observation" >&2; exit 2; }
...
Required gates: locked-restore=GREEN; release-build=GREEN; hk10-property-robustness=GREEN; hk10-endurance-compatibility=GREEN; regression=GREEN
Result: GREEN

...
[[ -z "$(git status --porcelain --untracked-files=all)" ]] || { echo "Candidate is not clean before HK09B observation" >&2; exit 2; }
...
DOTNET_NOLOGO=1 dotnet test tests/Arkus.Harness.Tests/Arkus.Harness.Tests.csproj \
  ...
  --filter 'FullyQualifiedName~Hk09B'
DOTNET_NOLOGO=1 dotnet test tests/Arkus.Harness.Tests/Arkus.Harness.Tests.csproj \
  --configuration Release --no-build --no-restore -m:1 --disable-build-servers

[[ -z "$(git status --porcelain --untracked-files=all)" ]] || { echo "Candidate is not clean after HK09B observation" >&2; exit 2; }
...
Canonical command: scripts/hk09b-observe-exact-sha.sh ${actual}
Candidate clean before: YES
Candidate clean after: YES
Required gates: locked-restore=GREEN; release-build=GREEN; resource-envelope=GREEN; persistence-interruption=GREEN; transport-conformance=GREEN; representative-content-shape-probe=GREEN; regression=GREEN
Result: GREEN

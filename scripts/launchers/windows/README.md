...
3. On first run, provide the local `Juego2` checkout path if needed, `TELEGRAM_BOT_TOKEN`, and numeric `TELEGRAM_CHAT_ID`.
4. The bot token is stored encrypted with Windows DPAPI for the current Windows user under `%LOCALAPPDATA%\Arkus\Juego2\console-launcher`.
5. Later runs verify Git/GitHub CLI/Codex/Python, refuse to modify a dirty checkout, fast-forward `main`, and launch `scripts/local_wp_remote_console.py`.

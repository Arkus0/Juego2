...
[[ -z "$(git status --porcelain --untracked-files=all)" ]] || { echo "Candidate is not clean before DW-00 verification" >&2; exit 2; }

bash scripts/dw00-observe-exact-sha.sh "${actual}"

grep -Fxq 'FOUNDATIONAL_PROOF_VERDICT: READY' Docs/evidence/WP-DW-00/PROOF_MATRIX.md
grep -Fxq 'UNRESOLVED_PROOF_OBLIGATIONS: 0' Docs/evidence/WP-DW-00/PROOF_MATRIX.md
grep -Fxq 'KNOWN_UNDETECTED_DEFECT_CLASSES: 0' Docs/evidence/WP-DW-00/PROOF_MATRIX.md
...
[[ -z "$(git status --porcelain --untracked-files=all)" ]] || { echo "Candidate is not clean after DW-00 verification" >&2; exit 2; }
...
Canonical command: scripts/dw00-verify-exact-sha.sh ${actual}
Candidate clean before: YES
Candidate clean after: YES
Required gates: locked-restore=GREEN; release-build=GREEN; focused-dw00=GREEN; reverse-h0-dependency=GREEN; generic-domain-boundary=GREEN; regression=GREEN; foundational-proof=GREEN; content-shape-probe=GREEN; residual-risk=GREEN; predecessor-check=GREEN; frozen-metadata=GREEN
Result: GREEN
Evidence: Docs/evidence/WP-DW-00
EOF

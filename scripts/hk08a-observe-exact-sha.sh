...
[[ -z "$(git status --porcelain --untracked-files=all)" ]] || { echo "Candidate is not clean before HK08A observation" >&2; exit 2; }
...
DOTNET_NOLOGO=1 dotnet test tests/Arkus.Harness.Tests/Arkus.Harness.Tests.csproj \
  ...
  --filter 'FullyQualifiedName~Hk08A'
DOTNET_NOLOGO=1 dotnet test tests/Arkus.Harness.Tests/Arkus.Harness.Tests.csproj \
  --configuration Release --no-build --no-restore -m:1 --disable-build-servers

[[ -z "$(git status --porcelain --untracked-files=all)" ]] || { echo "Candidate is not clean after HK08A observation" >&2; exit 2; }
...
Canonical command: scripts/hk08a-observe-exact-sha.sh ${actual}
Candidate clean before: YES
Candidate clean after: YES
Required gates: locked-restore=GREEN; release-build=GREEN; atomic-batching=GREEN; compact-bounded-reads=GREEN; journal-pagination=GREEN; cost-discovery=GREEN; causal-negative-controls=GREEN; regression=GREEN
Result: GREEN

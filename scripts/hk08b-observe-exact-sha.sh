...
[[ -z "$(git status --porcelain --untracked-files=all)" ]] || { echo "Candidate is not clean before HK08B observation" >&2; exit 2; }
...
[[ -z "$(git status --porcelain --untracked-files=all)" ]] || { echo "Candidate is not clean after HK08B observation" >&2; exit 2; }
...
Canonical command: scripts/hk08b-observe-exact-sha.sh ${actual}
Candidate clean before: YES
Candidate clean after: YES
Required gates: locked-restore=GREEN; release-build=GREEN; structured-recovery=GREEN; lineage-history-truth=GREEN; affected-only-reinspection=GREEN; diagnostic-preservation=GREEN; interaction-benchmark=GREEN; cross-transport=GREEN; causal-negative-controls=GREEN; regression=GREEN
Result: GREEN
